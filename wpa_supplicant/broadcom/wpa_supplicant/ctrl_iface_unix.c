...
wpa_supplicant_global_ctrl_iface_init(struct wpa_global *global)
{
	...
	wpa_printf(MSG_INFO, "Global control interface '%s'", ctrl);
	...
	if (os_strncmp(ctrl, "@abstract:", 10) == 0) {
		...
		wpa_printf(MSG_INFO, "Using Abstract control socket '%s'",
			   ctrl + 10);
		goto havesock;
	}
	...
	if (bind(priv->sock, (struct sockaddr *) &addr, sizeof(addr)) < 0) {
		...
		if (connect(priv->sock, (struct sockaddr *) &addr,
			    sizeof(addr)) < 0) {
			wpa_printf(MSG_INFO, "ctrl_iface exists, but does not"
				   " allow connections - assuming it was left"
				   "over from forced program termination");
			if (unlink(ctrl) < 0) {
				wpa_printf(MSG_ERROR,
	...
	wpa_printf(MSG_INFO, "Using UNIX control socket '%s'", ctrl);
	...
	if (global->params.ctrl_interface_group) {
		...
		if (grp) {
			gid = grp->gr_gid;
			wpa_printf(MSG_INFO, "ctrl_interface_group=%d"
				   " (from group name '%s')",
				   (int) gid, gid_str);
		} else {
			/* Group name not found - try to parse this as gid */
